...
samples = (open("R-Data/apicomplexa2.txt")
     |> readlines
...
# Map to hyperplane H
Hdist = sum.(vech.(samples))
n = length(vech(samples[1]))
pts_on_H = vech_to_matrix.(vech(samples[i]) - (Hdist[i]/n)*ones(n) for i in 1:length(samples))
samples = [phylogenetic_tree(pts_on_H[i], taxa(samples[i])) for i in 1:length(pts_on_H)]
